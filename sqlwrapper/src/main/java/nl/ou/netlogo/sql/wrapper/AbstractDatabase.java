 ...
 * Database engine dependent classes must implement this, and the DatabaseInfo
 * interface.
 * 
...
public abstract class AbstractDatabase implements DatabaseInfo, SqlConfigurable {
    ...
    /**
     * Convenience constructor to use an SqlSetting object to initialize this
     * object
     * 
     * @param settings
     *            should contain settings to make the database engine dependent
    ...
    /**
     * Initializes a database engine dependent class from the basic required
     * values
     * 
     * @param host
     *            host for the database engine
     * @param port
     *            network port for the database engine
     * @param user
     *            user to access the database engine
     * @param password
     *            password to access the database engine
     * @param schemaName
     *            schema/database name to access
     */
    protected AbstractDatabase(String host, int port, String user, String password, String schemaName) {
        this.host = host;
        this.port = port;
        this.user = user;
    ...
    public void configure(SqlSetting settings, Context context) throws Exception {
        if (settings.isValid()) {
            ...
            useAutoDisconnect = SqlSetting.toggleValue(settings
