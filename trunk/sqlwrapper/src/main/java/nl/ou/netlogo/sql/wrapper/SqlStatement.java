...
public class SqlStatement {
	...
	/**
	 * Constructor for SqlStatement with support for parameters (parameterized queries).
	 * 
	 * @param statement
	 *            PreparedStatement object
	 * @param parameters
	 *            Parameters as a NetLogo list (or null or empty list if no parameters are needed)
	 * @param autodisconnectCoordinator AutodisconnectCoordinator
	 */
	protected SqlStatement(PreparedStatement statement, LogoList parameters, AutodisconnectCoordinator autodisconnectCoordinator) {
	...
	public boolean executeDirect() throws ExtensionException {
		LOG.log(Level.FINE, "SqlStatement.executeDirect('{0}')",
	...
	public void executeQuery() throws ExtensionException {
		LOG.log(Level.FINE, "SqlStatement.executeQuery('{0}', '{1}')",
	...
	/**
	 * Executes a prepared update statement (eg INSERT, UPDATE, DELETE) through the JDBC interface.
	 * 
	 * @throws ExtensionException
	 */
	public void executeUpdate() throws ExtensionException {
		LOG.log(Level.FINE, "SqlStatement.executeUpdate('{0}', '{1}')",
				new Object[] { statement, parameters });
		try {
			prepareStatement();
	...
	protected void prepareStatement() throws SQLException, ExtensionException {
		...
		if ((parameters == null && parameterCount != 0)
			...
			throw new ExtensionException(
					String.format(
							"Incorrect number of query parameters passed, expected %d received %d",
							parameterCount,
	 ...
	 * @throws ExtensionException
	 */
	private void processParameter(Object parameter, int idx)
			throws SQLException, ExtensionException {
		...
		if (parameter instanceof String) {
			...
			throw new ExtensionException(String.format(
					"Unknown or unexpected parameter type %s", parameter.getClass().getName()));
		}
	}
