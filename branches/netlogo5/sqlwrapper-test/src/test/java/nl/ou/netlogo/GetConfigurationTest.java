...
public class GetConfigurationTest extends HeadlessTest {
     ...
     * </p>
     * <p>
     * Assumptions: expected defaults of test are equal to defaults configured
     * in SqlConfiguration class.
     * </p>
     * 
     * @throws Exception
    ...
    public void testGetConfiguration_defaultconnection_noconfig() throws Exception {
        ...
        for (int idx = 1; idx < resultList.size(); idx++) {
            ...
            assertTrue(String.format("Returned key %s not in expected defaults", key),
                    EXPECTED_DEFAULTS.containsKey(key));
    ...
    static {
        ...
        config.put("autodisconnect", "off");

        EXPECTED_CONFIG = Collections.unmodifiableMap(config);
    }

    /**
     * Test if sql:get-configuration reports right settings after setting values
     * using sql:configure.
     * <p>
    ...
    public void testGetConfiguration_defaultconnection_withConfig() throws Exception {
        ...
        workspace
                .command(String
                        .format("sql:configure \"defaultconnection\" [[\"host\" \"%s\"] [\"port\" \"%s\"] [\"user\" \"%s\"] [\"password\" \"%s\"] [\"database\" \"%s\"] [\"autodisconnect\" \"%s\"]]",
                                db.getHost(), db.getPort(), db.getUsername(), db.getPassword(), db.getSchema(), "off"));
        ...
        for (int idx = 1; idx < resultList.size(); idx++) {
            ...
            String key = (String) valuePair.get(0);
            String value = (String) valuePair.get(1);
            assertTrue(String.format("Returned key %s not in expected configuration", key),
                    EXPECTED_CONFIG.containsKey(key));
            assertEquals(String.format("Key %s has unexpected value", key), EXPECTED_CONFIG.get(key), value);
        }
    }

    /**
     * Test if sql:get-configuration throws an error if called with a
     * non-existent settingname.
     * 
     * @throws Exception
     *             For any exceptions during testing
     */
