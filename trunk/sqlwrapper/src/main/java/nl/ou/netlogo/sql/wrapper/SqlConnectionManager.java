 ...
 * 
 */
public class SqlConnectionManager implements SqlConfigurable,
		EventObserver<ConnectionEvent> {

	private static final Logger LOG = SqlLogger.getLogger();
	//
	// for any connection pool, a hard minimum count of connections should be available.
	// the wrapper will throw an exception when it is configured with a count that is effectively lower
	//
	...
	/**
	 * Method used to check if the connection manager is using a
	 * connection pool to manage physical database connections
	...
	public void initDefaultConnectionPool() {
		...
		try {
			...
			//
			// Make sure the correct driver is loaded before we
			// continue.
			//
			...
			if (connectionsPerPartition * partitions < MIN_CONNECTIONS) {
				String message = "Effective amount of connections is less than " + MIN_CONNECTIONS +
					". Configure max-connections and partitions so that there are at least " +
					MIN_CONNECTIONS + " connections in total.";
				LOG.severe(message);
				throw new ExtensionException(message);
			}
			...
			LOG.finest(String.format("Trying to access database '%s', user '%s', password '%s'", 
					dbInfo.getJdbcUrl(), dbInfo.getUser(), dbInfo.getPassword()));
	...
	/**
	 * Shuts down the existing connection pool, releasing connections etc in the process.
	 */
	 ...
	 */
	public SqlConnection getConnection(Agent agent, boolean createConnection)
			throws ExtensionException {
		...
		if (createConnection && connectionPool != null
				&& (sqlconn == null || !sqlconn.isConnected())) {
			// fetch a connection from the pool
			try {
				LOG.finest("SqlConnectionManager.getConnection(agent: " + agent + 
						", createConnection:" + createConnection + "): fetching a connection from the pool");
			...
			LOG.finest("SqlConnectionManager.getConnection(agent: " + agent + 
					", createConnection:" + createConnection + "): hitting the empty else branch. connectionPool: " +
					connectionPool);	
		}
	 ...
	 * @throws SQLException
	 */
	private SqlConnection createConnectionFromPool(Agent agent)
			throws SQLException {
		Connection conn = null;
		
		/**
		 * Implements logic to get a connection in a thread so that a timeout can be imposed
		 * @author NetLogo project-team
		 *
		 */
		class ConnectionGetter extends Thread {
			...
			public void run() {
				/*
				 * first obtain the lock, and notify the main thread it can continue 
				 */
				try {
		...
		/*
		 * Getting a connection from the pool can block forever if the pool is exhausted and no
		 * connections are closed. Try to obtain the connection in a thread, and impose a timeout
		 */
		...
		try {
			/*
			 * try to acquire the semaphore with a timeout. If the configured timeout is 0, don't try 
			 * to acquire the semaphore (and be willing to block indefinitely)
			 */
			if ((this.connectionPoolTimeout > 0 && binSem.tryAcquire(this.connectionPoolTimeout /*+ 2*/, TimeUnit.SECONDS)) 
				...
				/*
				 * obtaining the connection did not succeed within the timeout period, 
				 * terminate the thread and report failure
				 */
	 ...
	 * @throws ExtensionException
	 */
	public SqlConnection createConnection(Agent agent, String host, int port,
	 ...
	 * @throws ExtensionException
	 */
	public SqlConnection createConnection(Agent agent, String brand, String host, int port,
											String user, String password, String database)
			throws ExtensionException {
	 ...
	 * @throws ExtensionException
	 */
	public SqlConnection createConnection(Agent agent, DatabaseInfo myDbInfo)
			throws ExtensionException {
		//LOG.fine("Creating new unmanaged connection for Agent " + agent);
	 ...
	 * @throws ExtensionException
	 */
	public SqlConnection createConnection(DatabaseInfo myDbInfo)
			throws ExtensionException {
		try {
			Class.forName(myDbInfo.getDriverClass());
			Connection conn = DriverManager.getConnection(myDbInfo.getJdbcUrl(),
															myDbInfo.getUser(),
	...
	public void configure(SqlSetting settings, Context context) 
			throws DatabaseNotImplementedException, ExtensionException {
	...
	private void configureConnectionPool(SqlSetting settings) 
			throws ExtensionException {
		try {
			if (settings.isValid()) {
	...
	private void configureDatabase(SqlSetting settings, Context context)
	...
	private void remove(SqlConnection sqlConnection) {
		LOG.fine("Removing SqlConnection " + sqlConnection
	...
	public void notify(ConnectionEvent event,
			EventObservable<ConnectionEvent> observable) {
