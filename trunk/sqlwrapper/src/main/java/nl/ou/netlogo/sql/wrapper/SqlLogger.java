...
public class SqlLogger implements SqlConfigurable {
	...
	private class MyLogger extends Logger {
		...
		private void initializeLogger() {
			/*
			 * Create the logger. By default, set up a memory handler that will only push SEVERE messages
			 * to the console, (buffer size 1 makes it discard any other messages),
			 * and switch all parent handlers off
			 */
	...
	/* (non-Javadoc)
	 * @see {@link nl.ou.netlogo.sql.wrapper.SqlConfigurable#configure(nl.ou.netlogo.sql.wrapper.SqlSetting, org.nlogo.api.Context)}
	 */
		...
		if (settings.getName().equals(SqlConfiguration.LOGGING)) {
			...
			if (toggle) {
				if (fileHandler == null) {					
					 ...
					 */
					try {
						//fileHandler = new FileHandler(parseLogPath(settings.getString(SqlConfiguration.LOGGING_OPT_PATH), context) + "/sqlwrapper%g.log", 100000, 2, true);
						fileHandler = new FileHandler(parseLogPath(settings.getString(SqlConfiguration.LOGGING_OPT_PATH), context) + "/sqlwrapper.log");
			...
			logger.setCopy2StdErr(SqlSetting.toggleValue(settings.getString(SqlConfiguration.LOGGING_OPT_COPYTOSTDERR)));
	...
	private String parseLogPath(String path, Context context) throws Exception {
		if (context != null) {			
			/*
			 * special treatment: if path starts with "%m" or "%c", it is replaced by the model directory
			 * or the current directory of the context, respectively
			 */
	...
	/**
	 * Interprets a string as a Level value. Returns Level.ALL if the string could not be interpreted.
	 * 
	 * @param level
	 * @return LEVEL value
	 */
